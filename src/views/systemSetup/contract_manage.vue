<template>
  <div class="app-container">
    <div class="filter-container">
      <el-input v-model="listQuery.title" placeholder="员工名称" style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter" />
      <el-select v-model="listQuery.importance" placeholder="等级" clearable style="width: 90px" class="filter-item">
        <el-option v-for="item in importanceOptions" :key="item" :label="item" :value="item" />
      </el-select>
      <el-select v-model="listQuery.type" placeholder="门派" clearable class="filter-item" style="width: 130px">
    ...
    >
      ...
      <el-table-column label="入职时间" width="186px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.timestamp | parseTime('{y}-{m}-{d} {h}:{i}') }}</span>
        </template>
      </el-table-column>
      <el-table-column label="员工名称 / 门派" width="268px" align="center">
        <template slot-scope="{row}">
      ...
      <el-table-column label="职位" class-name="status-col" width="100">
        <template slot-scope="{row}">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="70px" style="width: 400px; margin-left:50px;" :disabled='isAdmin'>
        <el-form-item label="门派" prop="type">
          <el-select v-model="temp.type" class="filter-item" placeholder="选择用户门派">
        ...
        <el-form-item label="等级">
          <el-rate v-model="temp.importance" :colors="['#99A9BF', '#F7BA2A', '#FF9900']" :max="6" style="margin-top:8px;" />
        </el-form-item>
...
const calendarTypeOptions = [
  { key: '蜀山', display_name: '蜀山' },
  { key: '盛渔村', display_name: '盛渔村' },
  { key: '琼华派', display_name: '琼华派' },
  { key: '里蜀山', display_name: '里蜀山' }
]
...
export default {
  ...
  data() {
    return {
      ...
      pvData: [],
      rules: {
        type: [{ required: true, message: '请选择门派', trigger: 'change' }],
